...
public class UserService {
	...
	public User getUserById(long userId) {
		...
		return byId.orElseThrow(() -> new ResourceNotFoundException("could not find user with id %d".formatted(userId)));
	}

	public User getUserByEmail(String email) {
		Objects.requireNonNull(email, "email can not be null");
		Optional<User> byEmail = repository.findByEmail(email);
		return byEmail.orElseThrow(() -> new ResourceNotFoundException("could not find user with email %s".formatted(email)));
	}

	public User getUserByMobile(String mobile) {
		Objects.requireNonNull(mobile, "mobile can not be null");
		Optional<User> byEmail = repository.findByMobile(mobile);
		return byEmail.orElseThrow(() -> new ResourceNotFoundException("could not find user with mobile %s".formatted(mobile)));
	}
	...
	public UserResponse getUserResponseByEmail(final String email) throws UserException {
		return responseOf(getUserByEmail(email));
	}
